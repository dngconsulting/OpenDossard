...
const KEY_ORG = 'organization-signature-key';
...
describe('verifyHelloAssoSignature', () => {
  ...
  it('returns false when body is empty', () => {
    expect(verifyHelloAssoSignature(Buffer.alloc(0), [KEY], { 'x-ha-signature': 'abc' })).toBe(
      false,
    );
  });
  ...
  // --- Multi-clés (try-both) : la prod renvoie une signatureKey distincte par
  //     type de notification (Payment ≠ Organization). On accepte si la
  //     signature matche AU MOINS une des clés configurées.
  it('returns true when the signature matches the second of several keys', () => {
    const headers = { 'x-ha-signature': hmac(body.toString(), KEY_ORG) };
    expect(verifyHelloAssoSignature(body, [KEY, KEY_ORG], headers)).toBe(true);
  });

  it('returns false when the signature matches none of the provided keys', () => {
    const headers = { 'x-ha-signature': hmac(body.toString(), KEY_ORG) };
    expect(verifyHelloAssoSignature(body, [KEY, 'another-wrong-key'], headers)).toBe(false);
  });

  it('returns false when the keys array is empty', () => {
    const headers = { 'x-ha-signature': hmac(body.toString()) };
    expect(verifyHelloAssoSignature(body, [], headers)).toBe(false);
  });
});

...
const HorairesRace = (props: IRaceProps) => {
  ...
  const [competitionInfos, setCompetitionInfos] = useState<CompetitionInfo[]>(props.competition.competitionInfo);
  ...
  const onSubmit: SubmitHandler<CompetitionInfo> = async (data: CompetitionInfo) => {
    if (
      data.course === '' ||
  ...
  const deleteTabRow = (event: any): void => {
    ...
    props.updateCompetitionInfos(competitionInfos.splice(raceIndex, 1), infosError);
    props.onSaveCompetition(competitionInfos);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Tooltip
          title={
            "Si l'engagement/l'inscription à l'épreuve est sur place, donner l'heure d'engagement. S'il est en ligne, précisez du texte indiquant qu'il est en ligne."
          }
        >
          <TextField
            required={true}
      ...
      >
        ...
        <Tooltip
          title={
            "Attention à bien vérifier que le lien OpenRunner fonctionne, le profil du circuit sur Dossardeur est extrait à l'aide de ce lien"
          }
        >
          <TextField
      ...
      <TableContainer style={{ marginLeft: 16, marginTop: 16, padding: 0 }} component={Paper}>
        <Table className={classes.table} size="small" aria-label="a dense table">
          ...
          <TableBody>
            {competitionInfos && competitionInfos.length > 0 ? (
              ...
              <TableRow>
                <TableCell style={{ textAlign: 'center', height: 50 }} colSpan={8}>

    ...
    return <CompetitionLayout competitionId={competitionId}>
        {
            ({competition, currentRace, rows, fetchRows, fetchCompetition}) => {
                ...
                const getStats = () => {
                    ...
                    const groupByNbRidersByCatev = _.groupBy(lrows, (item: RaceRow) => item.catev);
                    const statsCates = Object.keys(groupByNbRidersByCatev).map(item => {
                        return {
                            "Catégorie": item,
                            "Nb de coureurs": groupByNbRidersByCatev[item].length
                        };
                    });
                    const nbCoureurs = [{"Nb engagés":lrows.length}]
                    const clubs = [{"Nb de clubs engagés":_.uniqBy(lrows, 'club').length}]
                    const cates = [{"Nb de caté.":_.uniqBy(lrows, 'catev').length}]
                    const depts = [{"Nb de départements":_.uniqBy(lrows, 'dept').length}]

                    return [...nbCoureurs,...statsCates,...clubs,...cates,...depts]
                }
                ...
                const exportPDF = async () => {
                    ...
                    _.orderBy(filterByRace(rows, currentRace), ['riderNumber'], ['asc']).forEach((r:RaceRow,index:number)=>{
                    ...
                    let statistiquesText = 'Statistiques : ' + JSON.stringify(getStats()).replace(/[\[\]"]+/g,'')
                    addWrappedText({
                        text: statistiquesText, // Put a really long string here
                        textWidth: 190,
                        fontSize:10,
                        doc,

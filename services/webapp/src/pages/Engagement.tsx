    ...
    return <CompetitionLayout competitionId={competitionId}>
        {
            ({competition, currentRace, rows, fetchRows, fetchCompetition}) => {
                ...
                const exportPDF = async () => {
                    ...
                    filterByRace(rows, currentRace).forEach((r:RaceRow,index:number)=>{
                        rowstoDisplay.push([r.riderNumber,r.name,r.club,r.gender,r.catev,r.catea])
                    })
                    let doc = new jsPDF("l","mm","a4");
                    // @ts-ignore
                    var totalPagesExp = '{total_pages_count_string}'
                    // @ts-ignore
                    doc.autoTable({head: [['Dossard', 'Nom et Prénom', 'Club','Genre','Caté.','Caté Age']],body: rowstoDisplay,
                        didDrawPage: (data:any) => {
                            ...
                            doc.text(competition.name  + ' - Coureurs engagés - Caté. ' + currentRace, data.settings.margin.left + 40, 30)
                            // Footer

            ...
            ({competition, currentRace, rows, fetchRows,fetchCompetition}) => {
                ...
                const exportPodiumsPDF = async () => {
                    ...
                    competition.races.forEach((currRace:string,index:number)=>{
                        ...
                        const rowsByCurrentRace = transformRows(filterByRace(rows, currRace))
                        // Put all main federation first
                        rowsByCurrentRace.forEach((r:RaceRow,index:number)=> {
                            if (rankOfCate(r, rowsByCurrentRace) <= 3 && r.fede === competition.fede)
                                (r.rankingScratch && r.comment==null) && podiumsForCurrentRace.push([rankOfCate(r, rowsByCurrentRace), r.rankingScratch, displayDossard(r.riderNumber.toString()), r.name, r.club, r.gender, r.catev, r.catea, r.fede])
                        })
                        ...
                        doc.setFontSize(12)
                        // @ts-ignore
                        doc.text('Course ' + currRace, 5, (index ==0 ? 25 : doc.autoTable.previous.finalY + 5));
                        // @ts-ignore
                        doc.autoTable({head: [['Cat.','Scrat.','Doss', 'Coureur', 'Club','H/F','Caté.V','Caté.A','Fédé']],
                            headStyles: {

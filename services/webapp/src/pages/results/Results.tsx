...
const EditResultsPage = (gprops: any) => {
  ...
  const fromPalmaresLicenceId = new URLSearchParams(gprops.location.search).get(
    "palmares"
  );
  ...
  const transformRows = (rows: RaceRow[]) => {
    ...
    return sortedByRanking.map((item: any, index: number) => {
      ...
      if (item.comment != null) {
        ...
        index === currentNotRankedStatus.rowindex
      ) {
      ...
      return {
        ...
        ...(item.rankingScratch !== null || item.comment !== null
          ? { ...item }
          : {})
      };
    });
  ...
  const dossardValidator = async (
    ...
    if (
      !isNaN(currentRanking) &&
      previousRowEmpty(allprops.rowIndex, transformedRows)
    ) {
      if (currentDossard && currentDossard.trim().length > 0) {
        setNotification({
          message:
            "Veuillez saisir le dossard " +
            currentDossard +
            " de manière consécutif aux autres",
          type: "error",
          open: true
        });
      }
    ...
    if (
      rows.filter(
        item =>
          item.riderNumber === parseInt(currentDossard) &&
          item.rankingScratch !== currentRanking
      ).length > 0
    ) {
      try {
        setLoading(true);
      ...
      if (currentDossard && currentDossard.trim().length > 0) {
        setNotification({
          message:
            ...
            currentDossard +
            " n'a pas été engagé ou est incorrect",
          type: "error",
          open: true
        });
        return false;
  ...
  const getMedalColorForRank = (
    ranking: number | string,
    rankingScratch: number
  ): string => {
    return ranking <= 3 || rankingScratch <= 3
      ? ["#efd807", "#D7D7D7", "#6A3805"][
          Math.min(rankingScratch, ranking as number) - 1
        ]
      : "#fff";
  };
      ...
      {({ competition, currentRace, rows, fetchRows, fetchCompetition }) => {
            ...
            if (rowdata.fede !== competition.fede) {
              rankToReturn =
                transformedRows
                  .filter(
                    (v: RaceRow) =>
                      v.fede !== competition.fede && v.catev === rowdata.catev
                  )
                  .findIndex((item: RaceRow) => item.id === rowdata.id) + 1;
            } else {
              rankToReturn =
        ...
        const AddWinnersIcons = (props: any) => {
          if (props.rowdata.rankingScratch) {
            const lrankOfCate: string | number = rankOfCate(
              props.rowdata,
              props.transformedRows
            );
            if (lrankOfCate <= 3 || props.rowdata.rankingScratch <= 3) {
              return (
                <Tooltip
                  title={getPodiumTitle(
                    lrankOfCate as number,
                    props.rowdata.rankingScratch
                  )}
                >
                  <EmojiEventsIcon
                    style={{
                      verticalAlign: "middle",
                      color: getMedalColorForRank(
                        lrankOfCate,
                        props.rowdata.rankingScratch
                      )
                    }}
        ...
        const getTitleChallengeButton = (row: RaceRow) => {
          return row.sprintchallenge
            ? "Enlever ce vainqueur du challenge"
            : "Ajouter comme vainqueur du challenge";
        };
        ...
        const notRankedEditor = (allprops: any) => {
          ...
          return (
            <select
              value={
                currentNotRankedStatus.status === ""
                  ? transformedRows[allprops.rowIndex].classement
                  : currentNotRankedStatus.status
              }
              onChange={(e: any) => {
        ...
        const getChallengeWinners = (filteredRows: any) => {
          const winners = filteredRows
            .filter((r: RaceRow) => r.sprintchallenge)
            .map((r: RaceRow) => r.name);
          if (winners.length === 0) return "NC";
          return winners.join(",");
        };
        const exportPodiumsPDF = async () => {
          const filename =
            "Podiums_" + competition.name.replace(/\s/g, "") + ".pdf";
            ...
            .forEach((currCategory: string, index: number) => {
              ...
              allRankRows.forEach((r: RaceRow) => {
                ...
                if (
                  ...
                  (r.catev === currCategory ||
                    (currCategory === "2" && r.catev === "1"))
                )
                  r.rankingScratch &&
                    r.comment == null &&
                ...
                if (r.sprintchallenge && r.catev === currCategory)
                  sprintChallenge = r;
              });
              const otherFedes = _.remove(
                podiumsForCurrentRace,
                (item: any) => item[8] !== competition.fede
              );
              const agregatedPodiums = podiumsForCurrentRace.concat(
                _.orderBy(otherFedes, item => item[1], ["asc"])
              );
              doc.setTextColor(40);
              doc.setFontSize(11);
              // @ts-ignore
              ...
              agregatedPodiums.length > 0 &&
                ...
                doc.autoTable({
                  ...
                  didDrawPage: (data: any) => {
                    ...
                    doc.text(
                      "Course(s) : " + competition.races.toString(),
                      data.settings.margin.left + 50,
                      25
                    );
                    // Footer
                    ...
                    const pageHeight = pageSize.height
                      ...
                      : pageSize.getHeight();
                    doc.text(
                      filename + " généré à " + moment().format("HH:mm"),
                      40,
                      pageHeight - 5
                    );
                  },
        ...
        const generateFilePDF = (races: string[]) => {
          ...
          races.forEach((currentRace: string, pageIndex: number) => {
            ...
            const filteredRowsByRace = transformRows(
              filterByRace(rows, currentRace)
            );
            ...
            doc.autoTable({
              ...
              didDrawPage: (data: any) => {
                ...
                doc.setFontSize(10);
                doc.text(
                  "Résultats générés avec Open Dossard (http://www.opendossard.com)",
                  50,
                  7
                );
                doc.setFontSize(13);
            ...
            doc.fromHTML(
              "<div><b>NOMBRE DE COUREURS :</b> " +
                ...
                (competition.competitionType === "CX"
                  ? "<br><b>ABOYEUR</b> : " +
                    (competition.aboyeur ? competition.aboyeur : "NC")
                  : "") +
                "<br><b>REMARQUES</b> : " +
          ...
          for (
            ...
            const pageHeight = pageSize.height
              ...
              : pageSize.getHeight();
            doc.text(
              "Page " + pageNumber + "/" + doc.internal.getNumberOfPages(),
              5,
              pageHeight - 10
            );
            doc.text(
          ...
          row.riderNumber && [
            ...
            row.sprintchallenge && (
              <Tooltip key={2} title={getTitleChallengeButton(row)}>
                <Delete
                  onClick={e => callFlagChallenge(row)}
                  key={2}
          ...
          <Fragment>
            {showDNFDialog && (
              ...
              >
                <Fragment>
                  <div>
                    Vous allez passer en mode Saisie des abandons, il vous
                    suffit de cliquer dans la colonne 'Classements' et de
                    sélectionner les motifs d'abandon dans la liste proposée.
            ...
            >
              <div style={{ display: "flex", flexDirection: "row" }}>
                <ActionButton
                  color="primary"
                  title="Visualiser ou éditer les classements"
                >
                  {isEdit ? (
                    <PageviewSharp
                ...
                {isEdit &&
                  (modeDNFActivated ? (
                    <ActionButton color={"secondary"}>
                      <a
                        style={{ color: "white" }}
                        onClick={e => setModeDNFActivated(false)}
                      >
                        Arrêter saisie des abandons
                      </a>
                    </ActionButton>
                  ) : (
                    <ActionButton
                      title="Saisir les déclassés, abandons, ..."
                    ...
                    >
                      ...
                      <a
                        style={{ color: "white" }}
                        onClick={e => setModeDNFActivated(true)}
                      >
                        Saisir les abandons
                      </a>
                    </ActionButton>
                ...
                <ActionButton
                  ...
                  onClick={(evt: React.MouseEvent) =>
                    setDownloadMenuAnchorEl(evt.currentTarget)
                  }
                >
                  <span style={{ color: "white" }}>
                    {" "}
                ...
                >
                  ...
                  <DropdownMenuItem
                    title={
                      "Télécharger les classements en PDF (Uniquement " +
                      currentRace +
                      ")"
                    }
                    onClick={() => {
              ...
              >
                ...
                >
                  {fromPalmaresLicenceId ? (
                    <Link
                      style={{ color: "#FFF" }}
                      to={"/palmares/" + fromPalmaresLicenceId}
                    >
            ...
            >
              ...
              <Column
                ...
                field="classement"
                header="Clt."
                {...(isEdit && modeDNFActivated
                  ? { editor: allprops => isEdit && notRankedEditor(allprops) }
                  : undefined)}
              ...
              <Column
                ...
                field="riderNumber"
                body={(row: RaceRow) =>
                  row.riderNumber && displayDossard(row.riderNumber.toString())
                }
                header="Dossard"
                filter={showFilters}
...
export default withStyles(null, { withTheme: true })(
  withRouter(EditResultsPage) as any
) as any;

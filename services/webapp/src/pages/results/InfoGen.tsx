...
import {CircularProgress, Grid, TextareaAutosize, withStyles} from "@material-ui/core";
...
const InfoGen = (props: InfoGenProps) => {
    ...
    useEffect(() => {
        const fetchCompetition = async () => {
            try {
                setShowSablier(true)
                setCurrentCompetition(await apiCompetitions.getCompetition({id: props.competitionId}))
            } finally {
    ...
    const saveInfoGen = () => {
        const save = async () => {
            setShowSablier(true)
            try {
                await apiCompetitions.saveInfoGen({competitionEntity: currentCompetition})
            }
            catch(err)
            {
                setNotification({
    ...
    return (
        ...
        >
            ...
            <DialogTitle style={{backgroundColor: '#2d4889', color: 'white'}} id="alert-dialog-title">Informations
                épreuve</DialogTitle>
                        ...
                        <Grid item xs={12}>
                            <FormControl className={classes.formControl}>
                                <TextField
                                    ...
                                    required={false}
                                    onChange={(e) => {
                                        setCurrentCompetition({...currentCompetition, speaker: e.target.value})
                                    }}
                                    value={currentCompetition && currentCompetition.speaker ? currentCompetition.speaker : ''}
                                    id="speaker"
                                    label="Speaker"
                                    margin="normal"
                                />
                            ...
                            {(currentCompetition?.competitionType === CompetitionEntityCompetitionTypeEnum.CX) &&
                            <FormControl className={classes.formControl}>
                              <TextField
                                value={currentCompetition && currentCompetition.aboyeur ? currentCompetition.aboyeur : ''}
                                onChange={(e) => {
                                    setCurrentCompetition({...currentCompetition, aboyeur: e.target.value})
                                }}
                                id="aboyeur"
                                label="Aboyeur CX"
                                margin="normal"
                            ...
                            <FormControl className={classes.formControl}>
                                ...
                                <TextareaAutosize
                                    ...
                                    onChange={(e) => {
                                        setCurrentCompetition({...currentCompetition, feedback: e.target.value})
                                    }}
                                    rowsMin={5}

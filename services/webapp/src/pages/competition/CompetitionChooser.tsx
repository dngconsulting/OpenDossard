...
const CompetitionChooser = (props: ICompetitionChooserProps) => {
  ...
  const displayClassement = (competition: Competition) => {
    const nbClasses = raceRows.filter(
      rr => rr.competitionId === competition.id && (rr.comment != null || rr.rankingScratch != null)
    ).length;
    return nbClasses > 0 ? (
  ...
  return (
    <Paper className={classes.root}>
      ...
      <div className={classes.titre}>
        {data?.length} épreuve(s) affichée(s), cliquez sur une ligne pour Engager/Classer
      </div>
      ...
      >
        <Column header="Engagements" body={displayEngagement} style={{ minWidth: '2%', textAlign: 'center' }} />
        <Column header="Classements" body={displayClassement} style={{ minWidth: '5%', textAlign: 'center' }} />
        <Column field="eventDate" header="Date" sortable={true} body={displayDate} style={{ minWidth: '2%' }} />
        <Column
          field="name"
        ...
        <Column
          ...
          body={(compRow: CompetitionEntity) => {
            return compRow.races.toString().substring(0, 20);
          }}
        />
        <Column
          field="fede"

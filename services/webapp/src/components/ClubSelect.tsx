...
export default function ClubSelect({dept, onSelectClubId, onSelectClubName, chosenClub, fede, isError}: IClubSelect) {
    // @ts-ignore
    ...
    const fetchData = async ()  => {
        ...
        if (chosenClub.label!=='' && chosenClub.value!==null) {
            club = loptionType.filter(option=>option.label.includes(chosenClub.label))[0]
        }
    ...
    const handleChangeSingle = (value: ValueType<IOptionType>) => {
        ...
        if(selected && selected.label) {
            onSelectClubName ? onSelectClubName(clubs.filter(club=>club.id===selected.value)[0].longName) : onSelectClubId(clubs.filter(club=>club.id===selected.value)[0].id)
        }
        else if(value === null) {
    ...
    const handleCreate = async (inputValue:string) => {
        setLoading(true)
        const newClub = await apiClubs.createClub({clubEntity:{id:0, shortName:null, dept, longName:inputValue, fede}})
        ...
        <FormControl className={classes.formControl}>
            <CreatableSelect isClearable={true}
                             ...
                             formatCreateLabel={(value:string) => (
                                 <div style={{flex:1, flexDirection:'row', padding:5, backgroundColor:'transparent'}}>
            ...
            {(isError && !selectedClub &&
                <FormHelperText error={true}>Le club doit être renseigné</FormHelperText>
            )}
        </FormControl>
    );
}

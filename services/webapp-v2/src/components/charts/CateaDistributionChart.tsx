...
export function CateaDistributionChart({ data, isLoading }: Props) {
  ...
  const chartConfig = Object.fromEntries(
    data.map((d, i) => [
      d.catea,
      { label: d.catea, color: COLORS[i % COLORS.length] },
    ]),
  ) satisfies ChartConfig;

  const chartData = data.map(d => ({ ...d, fill: chartConfig[d.catea]?.color }));
        ...
        <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[350px]">
          <PieChart>
            ...
            <Pie
              ...
              cy="50%"
              outerRadius={120}
              label={({ catea, percent }) => `${catea} (${(percent * 100).toFixed(0)}%)`}
              labelLine
            >
              {chartData.map((entry, index) => (
                <Cell key={entry.catea} fill={COLORS[index % COLORS.length]} />
              ))}
            </Pie>
            <ChartLegend content={<ChartLegendContent nameKey="catea" />} />
          </PieChart>
        </ChartContainer>
      </CardContent>
    </Card>

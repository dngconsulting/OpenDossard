...
export function CatevDistributionChart({ data, isLoading }: Props) {
  ...
  const handleExportPdf = () => {
    if (chartRef.current) exportChartToPdf(chartRef.current, 'Répartition par catégorie de valeur');
  };
  const { chartData, chartConfig, total, sortedAll } = useMemo(() => {
    ...
    const slices = [
      ...top.map((d, i) => ({ catev: d.catev, count: d.count, fill: COLORS[i % COLORS.length] })),
      ...(othersCount > 0 ? [{ catev: 'Autres', count: othersCount, fill: 'hsl(0, 0%, 70%)' }] : []),
    ];

    const config = Object.fromEntries(
      slices.map(d => [d.catev, { label: d.catev, color: d.fill }]),
    ...
    <CollapsibleChartCard
      ...
      header={
        <>
          ...
          <div>
            <CardTitle className="text-lg font-bold tracking-tight">Répartition par catégorie de valeur</CardTitle>
      ...
      <div ref={chartRef} className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <ChartContainer config={chartConfig} className="aspect-square w-full max-w-[260px] shrink-0">
          <PieChart>
            ...
            >
              {chartData.map((entry) => (
                <Cell key={entry.catev} fill={entry.fill} />
              ))}
              <Label
                content={({ viewBox }) => {
        ...
        <div className="w-full max-h-[280px] overflow-y-auto pr-1">
          <div className="space-y-1">
            {sortedAll.map((item, i) => {
              ...
              return (
                <div key={item.catev} className="flex items-center gap-2 text-xs group">
                  <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
                  <span className="truncate flex-1 text-foreground/80 group-hover:text-foreground font-medium">{item.catev}</span>
